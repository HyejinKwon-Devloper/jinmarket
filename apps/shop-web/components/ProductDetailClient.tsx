...
function formatGameProgress(
  progress: NonNullable<ProductDetail["myGameProgress"]>,
) {
...
export function ProductDetailClient({
  ...
  async function refreshProductDetail() {
    const refreshed = await requestJson<{ item: ProductDetail }>(
      `/products/${productId}`,
    );
    ...
    <section className="detailGrid">
      ...
      <div className="panel detailInfoPanel">
        <div className="badgeRow">
          <span className={`badge ${isOpen ? "success" : ""}`}>
        ...
        {isSeller ? (
          <div className="message">
            내가 등록한 상품입니다. 신청이나 가격 제안은 다른 계정으로만
            가능합니다.
          </div>
        ) : null}
        {isOpen && isSaleNotStarted ? (
          <div className="message">
            아직 판매 시작 전인 상품입니다. 판매 시작 일시 이후에 다시 확인해
            주세요.
          </div>
        ) : null}
        {isOpen && isSaleEndedByPeriod ? (
          <div className="message">
            판매 기간이 종료된 상품입니다. 더 이상 구매나 가격 제안을 진행할 수
            없습니다.
          </div>
        ) : null}
        ...
        {item.purchaseType === "INSTANT_BUY" && !isSeller ? (
          <div className="actionRow" style={{ marginTop: 16 }}>
            {isActionLocked ? (
              ...
              >
                {isPurchasing
                  ? `${item.isFreeShare ? "신청" : "구매"} 처리 중...`
                  : `바로 ${item.isFreeShare ? "신청" : "구매"}`}
              </button>
            ) : null}
          </div>
        ) : null}
        ...
        {item.purchaseType === "GAME_CHANCE" && !isSeller ? (
          <div style={{ marginTop: 16 }}>
            {gameProgressMessage ? (
              <div className="message">{gameProgressMessage}</div>
            ) : null}
            {isActionLocked ? (
        ...
        {item.myGameAttempt && !gameProgressMessage ? (
          <div className="message">
            최근 가위바위보 결과:{" "}
            {gameChoiceLabels[item.myGameAttempt.playerChoice]} / 상대{" "}
            {gameChoiceLabels[item.myGameAttempt.systemChoice]} /{" "}
            {formatGameResult(item.myGameAttempt.result)}
          </div>
        ) : null}
      ...
      <GamePurchaseModal
        ...
        onPlay={async (choice: GameChoice) => {
          const result = await requestJson<GamePlayResult>(
