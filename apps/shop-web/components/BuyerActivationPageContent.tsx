...
export function BuyerActivationPageContent() {
  ...
  async function handleRequestCode(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      setSubmitting(true);
      setMessage(null);
      const response = await requestJson<ActivationResponse>("/auth/buyer-activate/request-code", {
        method: "POST",
        body: JSON.stringify({
          loginId,
          email,
  ...
  async function handleVerify(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      setSubmitting(true);
      setMessage(null);
      const response = await requestJson<ActivationResponse>("/auth/buyer-activate/verify", {
        method: "POST",
        body: JSON.stringify({
          loginId,
          email,
